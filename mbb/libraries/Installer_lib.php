...
class Installer_lib
{
     ...
     */
    public function setup($post_data)
    {
    ...
    ********************************************************************************/
    ...
    function write_config() {
    	...
    	$hostname	= $this->ci->input->post('db_hostname');
    	$username	= $this->ci->input->post('db_username');
    	$password	= $this->ci->input->post('db_password');
    	$db_name	= $this->ci->input->post('db_name');
    
    
    	// Config path
    	$template_path 	= '../application/db/config/database.php';
    ...
    function create_database()
    {
    	...
    	$password	= $this->ci->input->post('db_password');
    	$db_name	= $this->ci->input->post('db_name');
    	$mysqli = new mysqli($hostname, $username, $password, ''); // Connect to the database
    	if ( ! $mysqli->connect_error) // Check for errors
    	{
    ...
    function create_tables()
    {
    	...
    	$templine = '';
    	// Read in entire file
    	$lines = file('../application/db/mbb.sql'); // Open the default SQL file
    	...
    	foreach ($lines as $line)
    	{
    		...
    		{
    			// Perform the query
    			$mysqli->multi_query($templine); // Execute a multi query
    ...
    /**
     * Upgrade mbb to the latest version
     * @return string|boolean
     */
    public function upgrade() {
    	$this->ci->hooks->enabled = false; // Disable Hooks because of Error
