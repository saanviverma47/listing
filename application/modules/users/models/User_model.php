...
class User_model extends BF_Model
{
    ...
    public function prep_data($post_data)
    {
        ...
        // Save mobile number
        $data['mobile_number'] = $post_data['mobile_number'];
    ...
    public function set_activation($user_id)
    {
        ...
        switch ($activation_method) {
            case 0:
                ...
                $email_mess = $this->email_content(4, site_url());
                break;
            case 1:
                // Email Activiation.
                ...
                $email_mess = $this->email_content(3, $activate_link, $activation_code);
                break;
            case 2:
                // Admin Activation.
                $ccAdmin   = true;
                ...
                //Get email content from database
                $email_mess = $this->email_content(5);
                break;
        }
        ...
        $data = [
            ...
            'subject' => $email_mess['subject'],
            'message' => $email_mess['message'],
        ];
     ...
     */
    public function email_content($id, $link = NULL, $code = NULL ) {
    	$this->load->model('email_templates/email_templates_model');
    	// Get email content from database
    	$email_template = $this->email_templates_model->select('subject, message')->find($id);
    
    	// Load parser library to format message
    	...
    	$message = $this->parser->parse('email_templates/email_body', $message_data, TRUE);
