...
?>
	<div class="pull-right bottom2">
	<?php echo anchor(site_url('members/add_classified?id=' .$this->input->get('id')), '<span class="glyphicon glyphicon-plus"></span>' .lang('new') .' '.lang('label_classified'), 'class="btn btn-primary pull-right"'); ?>
	</div><br />
	<?php echo form_open($this->uri->uri_string.'?'.$_SERVER['QUERY_STRING']); ?>
		<table class="table table-striped">
			...
			<tbody>
				...
				<?php if ($can_edit) : ?>
					<td><?php echo anchor(site_url('members/edit_classified?id=' . $this->encrypt->encode($record->id)), '<span class="icon-pencil"></span>' .  $record->title); ?></td>
				<?php else : ?>
