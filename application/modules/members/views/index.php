			...
			<tbody>
				...
				<?php if ($can_edit) : //urlencode(base64_encode($record->id))?>
					...
					<div class="hover-item small">
							...
							<a href="<?php echo site_url('members/logo?id=' . $this->encrypt->encode($record->id)); ?>" title="Logo"><?php echo lang('listings_action_logo'); ?></a> |
							<a href="<?php echo site_url('members/products?id=' . $this->encrypt->encode($record->id)); ?>" title="Product or Service"><?php echo lang('listings_action_product_service'); ?></a> |
							<a href="<?php echo site_url('members/photos/'.$record->id.'/?id=' . $this->encrypt->encode($record->id)); ?>" title="Photos"><?php echo lang('listings_action_photos'); ?></a> |
							<a href="<?php echo site_url('members/videos?id=' . $this->encrypt->encode($record->id)); ?>" title="Videos"><?php echo lang('listings_action_videos'); ?></a> |
							<a href="<?php echo site_url('members/classifieds?id=' . $this->encrypt->encode($record->id)); ?>" title="Classifieds"><?php echo lang('listings_action_classifieds'); ?></a> |
							<a href="<?php echo site_url('members/business_hours?id=' . $this->encrypt->encode($record->id)); ?>" title="Business Hours"><?php echo lang('listings_action_working_hours'); ?></a>
					</div></td>
				<?php else : ?>
