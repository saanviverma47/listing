	...
	<?php echo form_open($this->uri->uri_string(), 'class="form-horizontal"'); ?>
		<fieldset>
			...
			<div class="control-group <?php echo form_error('name') ? 'error' : ''; ?>">
				<div class='controls'>
					<input id='states_abbrev' type='text' name='states_abbrev' maxlength="2" class='span2' placeholder='<?php echo sprintf(lang('placeholder_code'), strtolower(lang('label_state')));?>' value="<?php echo set_value('states_abbrev', isset($states['abbrev']) ? $states['abbrev'] : ''); ?>" />
					<span class='help-inline'><?php echo form_error('abbrev'); ?></span>
					<input id='states_name' type='text' name='states_name' maxlength="40" class='span4' placeholder='<?php echo sprintf(lang('placeholder_location_name'), strtolower(lang('label_state')));?>' value="<?php echo set_value('states_name', isset($states['name']) ? $states['name'] : ''); ?>" />
		...
		<table class="table table-striped">
			<thead>
				<tr>
					...
					<th><?php echo lang('label_country') .' '.lang('label_iso');?></th>
			...
			<tbody>
				...
				<?php if ($can_edit) : 
					if($this->uri->segment(5) && ($this->uri->segment(6)) && ($this->uri->segment(7))): ?>
						<td><?php echo anchor(SITE_AREA .'/settings/locations/states/'.$this->uri->segment(5).'/'.$this->uri->segment(6).'/'.$this->uri->segment(7).'/'. $record->id, $record->name);?></td>
					<?php elseif($this->uri->segment(5) && ($this->uri->segment(6))): ?>
						<td><?php echo anchor(SITE_AREA .'/settings/locations/states/'.$this->uri->segment(5).'/'.$this->uri->segment(6).'/0/'. $record->id, $record->name);?></td>
					<?php else:?>
						<td><?php echo anchor(SITE_AREA .'/settings/locations/states/'.$this->uri->segment(5).'/all/0/'. $record->id, $record->name);?></td>
					<?php endif;?>
				<?php else : ?>
					<td><?php e($record->name); ?></td>
				<?php endif; ?>
					<td><?php e($record->abbrev) ?></td>
					<td><?php e($record->country_iso) ?></td>
					<td>
					...
					<td>
					<?php echo anchor(SITE_AREA . '/settings/locations/cities/' . $record->id, '<span class="label label-info">'.lang('label_view'). ' ' .lang('label_cities').'</span>')?></td>
				</tr>
				<?php
					endforeach;
				else:
