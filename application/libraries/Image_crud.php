...
class image_CRUD {
	...
	/* mbb.com Changes */
	protected $active_field = null; // to change active field value
	protected $active_field_value = null; //Active field value
	protected $user_limit = null; //Set user limit parameter
	protected $images_count = null; // get total images uploaded by the user
	protected $user_role = null; // get user role id to display change status
	/* End of mbb.com Changes */
	...
	/* mbb.com Changes */
	// set active field
	function set_active_field($active_field)
	{
	...
	// set active field value as per package
	public function active_field_value($active_field_value)
	{
		$this->active_field_value = $active_field_value;
	
		return $this;
	}
	
	// maximum images user can upload
	public function user_limit($limit_value)
	{
		$this->user_limit = $limit_value;
	
		return $this;
	}
	
	// get images count from database to compare it with user_limit
	public function images_count($count_value)
	{
		$this->images_count = $count_value;
	
		return $this;
	}
	
	// display change status checkbox based on user permission
	public function user_role($role_value)
	{
		$this->user_role = $role_value;
    ...
    {
		$this->ci->db->update($this->table_name, array($this->title_field => escape($value)), array($this->primary_key => $primary_key));
    }
    
    /* mbb.com Changes */
    // make image active or inactive on admin click
    protected function _update_status($primary_key, $value)
    {
    	$this->ci->db->update($this->table_name, array($this->active_field => $value), array($this->primary_key => $primary_key));
    }
    /* End of mbb.com Changes */
    ...
    protected function _insert_table($file_name, $relation_id = null)
    {
    	...
    	/* mbb.com Changes */
    	// set active field value defined in package
    	$insert[$this->active_field] = $this->active_field_value;
    	/* End of mbb.com Changes */
    ...
    protected function _get_delete_url($value)
    {
		...
		/* mbb.com Changes */
    	// admin image delete url
    	if(($this->user_role == 1) && ($this->ci->uri->segment(1) != 'members')) {
    		$segments_array = $this->ci->uri->segment_array();
    		$rsegments_array[1] = $segments_array[1].'/'.$segments_array[2].'/'.$segments_array[3];    	
    	}
    	/* End of mbb.com Changes */
	...
	protected function getState()
	{
		...
		if(isset($rsegments_array[3]) && is_numeric($rsegments_array[3]))
		{
			...
			/* mbb.com Changes */
			$update_status_url  = site_url($rsegments_array[1].'/'.$rsegments_array[2].'/update_status');
			/* End of mbb.com */
			...
			/* mbb.com Changes */
			$state['update_status_url'] = $update_status_url;
			/* End of mbb.com */
		...
		/* mbb.com Changes */
		// Update Status
		elseif(isset($rsegments_array[3]) && $rsegments_array[3] == 'update_status')
		{
			$state = array( 'name' => 'update_status');
			return (object)$state;
		}
		/* End of mbb.com Changes */
	}
			...
			switch ($state_info->name) {
				case 'list':
					...
					$this->_library_view('list.php',array(
						...
						/* mbb Changes */
						// Pass paramters to library and list view
						'active_field' => $this->active_field,
						'update_status_url' => $state_info->update_status_url,
						'active_field_value' => $this->active_field_value,
				...
				case 'upload_file':
					...
					/* mbb Changes */
					// display message to the user when they reach their limit
					if(($this->user_limit != 0) && ($this->user_limit) <= ($this->images_count))
					{
						$info=lang('upload_limit');
						header('Content-type: application/json');
						echo json_encode((object)array('error' =>$info));
						die();
					}
					/* End of mbb Changes */
				...
				case 'update_status':
					@ob_end_clean();
					$this->_update_status($_POST['primary_key'],$_POST['value']);
					die();
				break;
				/* End of mbb.com Changes */
			}
...
class ImageUploadHandler
{
	...
	function __construct($options=null) {
		...
		// take precedence over the following max_file_size setting:
				'max_file_size' => settings_item('lst.image_file_size') * 1024, //mbb.com Changes
				'min_file_size' => 1,
				'accept_file_types' => '/.+$/i',
				...
				'orient_image' => false,
				/* mbb.com Changes */
				// set image maximum and minimum pixels
				'max_width' => settings_item('lst.image_width'),
				'max_height' => settings_item('lst.image_height'),
				'min_width' => 10,
				'min_height' => 10,
				/* End of mbb.com Changes */
	...
	private function handle_file_upload($uploaded_file, $name, $size, $type, $error) {
		...
		if (!$error && $file->name) {
			...
			$file->error = $error;
			/* mbb.com Changes */
			// if image size and pixels exceed display message to the user 
			header('Content-type: application/json');
			echo json_encode((object)array('error' =>lang('upload_failed')));
			die();
			/* End of mbb.com Changes */
		}
		return $file;
